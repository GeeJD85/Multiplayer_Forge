...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] PlayerConnectionNetworkObject = null;
		public GameObject[] PlayerControlNetworkObject = null;
		public GameObject[] PlayerNamePlateNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is PlayerConnectionNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerConnectionNetworkObject.Length > 0 && PlayerConnectionNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerConnectionNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerConnectionBehavior>();
							go.SetActive(false);
						}
					}
			...
			else if (obj is PlayerControlNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerControlNetworkObject.Length > 0 && PlayerControlNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerControlNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerControlBehavior>();
							go.SetActive(false);
						}
					}
			...
			else if (obj is PlayerNamePlateNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerNamePlateNetworkObject.Length > 0 && PlayerNamePlateNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerNamePlateNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerNamePlateBehavior>();
							go.SetActive(false);
						}
					}
		...
		/// <summary>
		/// Instantiate an instance of PlayerConnection
		/// </summary>
		/// <returns>
		/// A local instance of PlayerConnectionBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerConnection prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerConnectionBehavior InstantiatePlayerConnection(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (PlayerConnectionNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: PlayerConnection. Add them at the NetworkManager prefab.");
				return null;
			}
			
			var go = Instantiate(PlayerConnectionNetworkObject[index]);
			go.SetActive(false);
			var netBehavior = go.GetComponent<PlayerConnectionBehavior>();
			...
			go.GetComponent<PlayerConnectionBehavior>().networkObject = (PlayerConnectionNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of PlayerControl
		/// </summary>
		/// <returns>
		/// A local instance of PlayerControlBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerControl prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerControlBehavior InstantiatePlayerControl(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (PlayerControlNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: PlayerControl. Add them at the NetworkManager prefab.");
				return null;
			}
			
			var go = Instantiate(PlayerControlNetworkObject[index]);
			go.SetActive(false);
			var netBehavior = go.GetComponent<PlayerControlBehavior>();
			...
			go.GetComponent<PlayerControlBehavior>().networkObject = (PlayerControlNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of PlayerNamePlate
		/// </summary>
		/// <returns>
		/// A local instance of PlayerNamePlateBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerNamePlate prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerNamePlateBehavior InstantiatePlayerNamePlate(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (PlayerNamePlateNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: PlayerNamePlate. Add them at the NetworkManager prefab.");
				return null;
			}
			
			var go = Instantiate(PlayerNamePlateNetworkObject[index]);
			go.SetActive(false);
			var netBehavior = go.GetComponent<PlayerNamePlateBehavior>();
			...
			go.GetComponent<PlayerNamePlateBehavior>().networkObject = (PlayerNamePlateNetworkObject)obj;
