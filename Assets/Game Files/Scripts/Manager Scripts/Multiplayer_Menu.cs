	...
	public class Multiplayer_Menu : MonoBehaviour
	{
		...
		public void Host()
		{
			...
			else
			{
				...
				if (natServerHost.Trim().Length == 0)
					((UDPServer)server).Connect(iPAddress.ToString(), (ushort)15940);
				else
					((UDPServer)server).Connect(port: ushort.Parse(portNumber.ToString()), natHost: natServerHost, natPort: natServerPort);
			}
